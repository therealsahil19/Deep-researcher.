...
-   **Autonomous Web Search**: The agent can autonomously perform web searches to find real-time data using:
    -   **Exa (Metaphor)**: For broad discovery and landscape analysis.
    -   **Tavily**: For specific fact retrieval and verification.
...
-   (Optional) A [Tavily](https://tavily.com/) API Key for fact-based search.
-   (Optional) An [Exa](https://exa.ai/) API Key for broad discovery search.

## Project Structure

-   `app.py`: The main entry point for the Streamlit application, handling the UI and state management.
-   `utils.py`: Contains utility functions for API interaction (`stream_deep_research`), Tavily/Exa search integration, and PDF generation (`generate_pdf`).
...
2.  **Configure the Agent:**
    ...
    -   (Optional) Enter your **Tavily API Key** to enable fact checking.
    -   (Optional) Enter your **Exa API Key** to enable broad discovery.

3.  **Start Researching:**
    -   Select your preferred mode: **Simple Report Generator** or **Chat Interface**.
    -   Enter your research topic or question.
    -   Wait for the model to generate the insights. You may see the agent performing "Discovery Search" or "Fact Search" actions.
...
-   **Search Not Working**: Ensure you have provided valid Tavily or Exa API Keys in the sidebar for the respective actions.

## Technology Stack

-   **Frontend**: [Streamlit](https://streamlit.io/)
-   **Model**: `alibaba/tongyi-deepresearch-30b-a3b:free` (via OpenRouter)
-   **Web Search**: [Tavily API](https://tavily.com/) & [Exa API](https://exa.ai/)
